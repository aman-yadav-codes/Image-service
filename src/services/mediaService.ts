...
export async function uploadMedia(input: MediaUploadInput): Promise<MediaResponse> {
  ...
  if (input.size > config.media.maxFileSizeBytes) {
    throw AppError.payloadTooLarge(`File size ${input.size} bytes exceeds the media limit of ${config.media.maxFileSizeBytes} bytes`);
  }
  ...
  const metadata: MediaMetadata = {
    id, kind, status: 'queued', originalFilename,
    originalMimeType: input.mimetype, originalSizeBytes: input.size,
    createdAt: now, updatedAt: now, completedVariants: [], variants: {},
  };
...
export async function markMediaVariantCompleted(id: string, variant: MediaMetadata['completedVariants'][number], filename: string): Promise<void> {
  ...
  if (required.every((v) => meta.completedVariants.includes(v as typeof variant))) meta.status = 'completed';
  else meta.status = 'processing';
...
function toResponse(meta: MediaMetadata): MediaResponse {
  const variants = Object.fromEntries(Object.entries(meta.variants).map(([name, filename]) => [name, `/media/${meta.id}/${filename}`]));
